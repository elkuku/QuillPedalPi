...
print("{{\"timestamp\":{timestamp},\"heartbeat\":{heartbeat}}}".format(**locals()))
...
if __name__ == '__main__':
    ...
    while True:
        # read from the ADC
        Signal = adc.read_adc(3, gain=GAIN)   #TODO: Select the correct ADC channel. I have selected A0 here
        ...
        if N > 250 :                                   # avoid high frequency noise
            if  (Signal > thresh) and  (Pulse == False) and  (N > (IBI/5.0)*3.0)  :
              ...
              text_file = open("heartbeat.txt", "w")
              text_file.write("{{\"timestamp\":{timestamp},\"heartbeat\":{heartbeat}}}".format(timestamp=timestamp,heartbeat=BPM))
              text_file.close()
        ...
        if N > 2500 :                          # if 2.5 seconds go by without a beat
            ...
            text_file = open("heartbeat.txt", "w")
            text_file.write("{{\"timestamp\":{timestamp},\"heartbeat\":{heartbeat}}}".format(timestamp=timestamp,heartbeat=0))
            text_file.close()
